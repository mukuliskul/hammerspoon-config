...
function obj:_buildArrangement()
	...
	for _, v in pairs(hs.screen.allScreens()) do
		...
		for k, wv in pairs(windows) do
			...
			if k == 1 then
				local buttonPressed, name =
					hs.dialog.textPrompt("Name this monitor", "", "e.g., " .. v:name(), "OK", "Cancel")
...
function obj:createArrangement()
	...
	local continue = hs.dialog.blockAlert(
		'Welcome to "Arrange Desktop"!',
	...
	local buttonPressed, arrangementName =
		hs.dialog.textPrompt("Name this Desktop Arrangement:", "", "e.g., Office", "OK", "Cancel")
	...
	hs.dialog.blockAlert(
		...
		"Each window will flash into focus. The first focus on each monitor will prompt you to name the monitor."
	)
	...
	hs.dialog.blockAlert(
		"Your desktop arrangement has been saved!",
		"Make sure to check your configuration for any duplicate application windows.",
		"OK"
	)
end
